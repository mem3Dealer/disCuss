...
class RoomCubit extends Cubit<RoomState> {
  ...
  Future<void> loadChat(String groupId) async {
    ...
    data.chatStream(groupId).listen((result) {
      List<Message>? _msg = result.docs.map<Message>((e) => Message.fromSnapshot(e)).toList();
      final _messages = _msg.reversed.toList();
      for (int i = 0; i < _messages.length; i++) {
        print('$i ${_messages[i].sender?.name}');
        if (i > 0 && i < _messages.length - 1) {
          _messages[i].isLast = _messages[i].sender?.uid != _messages[i + 1].sender?.uid;
        } else {
        ...
        if (i > 0 && i < _messages.length) {
          _messages[i].isFirst = _messages[i - 1].sender?.uid != _messages[i].sender?.uid;
        } else {
      ...
      emit(state.copyWith(
          currentRoom: newCurrentRoom?.copyWith(chatMessages: _listOfMessages), version: state.version! + 1));
